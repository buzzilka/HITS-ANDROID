...
class Gauss {
    suspend fun gaussianBlur(image: Bitmap, radius: Int, progressCallback: () -> Unit): Bitmap =
        coroutineScope {
                ...
                async(Dispatchers.Default) {
                    ...
                    for (y in startY until endY) {
                        for (x in 0 until width) {
                            ...
                            blurredChunk[(y - startY) * width + x] =
                                (0xFF shl 24) or (redSum.toInt() shl 16) or (greenSum.toInt() shl 8) or blueSum.toInt()
                        }
                    }
                    blurredChunk
                }
